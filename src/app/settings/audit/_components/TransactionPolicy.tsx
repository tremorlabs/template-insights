...
const getStateColor = (state: string) => {
  const category = keywordCategories.find((category) => category.value === state);
      ...
      <div className="grid grid-cols-1 gap-x-14 gap-y-8 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          ...
          <ul role="list" className="mt-6 flex flex-wrap gap-12">
            {blacklist.map((item) => (
              <li key={item.category} className="flex items-start gap-2.5">
                <span className={cx(item.color, "mt-[2px] size-2.5 rounded-sm")} aria-hidden="true" />
                <div>
          ...
          <ul role="list" className="mt-1 divide-y divide-gray-200 dark:divide-gray-800">
            {keywords.map((item) => (
              <li key={item.value} className="flex items-center justify-between py-2.5">
          ...
          >
            <div
              className={cx("animate-slideDownAndFade transition", isSpendMgmtEnabled ? "hidden" : "")}
              style={{
                    ...
                    <SelectTrigger className="w-48">
                      <SelectValue aria-label={value}>
                        <div className="flex gap-2 items-center">
                          <div className={cx("size-3 shrink-0 rounded", getStateColor(value))} />
                          <p>{value}</p>
                        </div>
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
                      {keywordCategories.map((item) => (
                        <SelectItem key={item.value} value={item.value}>
                          <div className="flex gap-2 items-center">
                            <div className={cx("size-3 shrink-0 rounded", item.color)} />
                            <p>{item.value}</p>
                          </div>
                          <span className="text-sm font-normal text-gray-700 ml-5">{item.description}</span>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
